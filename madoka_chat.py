...
def main():
    ...
    if auth_status is False:
        ...
        st.info("更新了新的好感度攻略系统（亲密开放度),用户可以通过聊天来增长或降低亲密开放度，不同程度的亲密开放度会有不同的态度回复。#开放了访客登入，优化了提示词与性格补充不生效的问题，现在用户可以利用以上账号登入系统进行聊谈，关闭了访客删除记录与直接使用后门增加好感度的功能，鼓励各位多聊天,新增土味情话识别系统，各位谨慎发言,更新云端数据库存储会话记录")
        st.stop()
